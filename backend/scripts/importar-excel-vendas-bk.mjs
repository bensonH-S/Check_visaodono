/**
 * Import rápido (bulk) do Excel Produto Venda → DB.
 *   node backend/scripts/importar-excel-vendas-bk.mjs --file="..." --bkn=23531
 */
...
const bkn = String(args.bkn || '').replace(/\D/g, '');
...
const loja = lojas[0];
const itens = parseVendasExcelBuffer(fs.readFileSync(file), { bkNumber: bkn });
const total = itens.reduce((a, r) => a + (Number(r.venda_liquida) || 0), 0);
console.log(
  `${loja.name} — ${itens.length} linhas — Bruto R$ ${total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
);
...
// limpa só esta loja bkoffice (caso import parcial anterior)
await client.query(
...
const { rows: sum } = await client.query(
  `SELECT COALESCE(SUM(vi.venda_liquida),0)::float AS v, COUNT(DISTINCT v.data_venda)::int AS dias
...
console.log(
  `OK em ${Math.round((Date.now() - t0) / 1000)}s — banco R$ ${Number(sum[0].v).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} · ${sum[0].dias} dias · ${idVenda.length} itens`,
);
await client.end();
