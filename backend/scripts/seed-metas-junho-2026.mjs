/**
 * Importa metas de junho/2026 a partir da planilha METAS JUNHO 2026.xlsx
 * Uso: node backend/scripts/seed-metas-junho-2026.mjs [caminho.xlsx]
 */
import fs from 'fs';
...
const XLSX_PATH =
  process.argv[2] ||
  path.join(process.env.USERPROFILE || '', 'Downloads', 'METAS JUNHO 2026.xlsx');
...
async function importarPremios(client, idPeriodo, sheet) {
  ...
  for (let r = 2; r <= 13; r += 1) {
    ...
    const premio_saude = row[1] != null ? Number(row[1]) : null;
    const premio_rev = row[2] != null ? Number(row[2]) : null;
    const valor_unitario = row[3] != null ? Number(row[3]) : null;
    const subtotal = row[4] != null ? Number(row[4]) : null;
    const total = row[5] != null ? Number(row[5]) : null;
...
const RANKINGS = [
  ...
  { codigo: 'rank_misterioso', nome: 'Cliente Misterioso', ordem: 40, row_start: 38, row_end: 57, col_pos: 0, col_loja: 1, col_valor: 2, col_pts: 3, col_classe: null, meta_minima: 0.85 },
  { codigo: 'rank_delivery', nome: 'Delivery iFood', ordem: 50, row_start: 38, row_end: 57, col_pos: 5, col_loja: 6, col_valor: 7, col_pts: 8, col_classe: null, meta_minima: 4.8 },
  { codigo: 'rank_ano_anterior', nome: 'Ano - 1', ordem: 60, row_start: 38, row_end: 57, col_pos: 9, col_loja: 10, col_valor: 11, col_pts: 12, col_classe: null, meta_minima: 0.1 },
...
try {
  ...
  try {
    ...
    console.log(`\nOK — período #${idPeriodo} (Junho/2026) importado.`);
  } catch (e) {
