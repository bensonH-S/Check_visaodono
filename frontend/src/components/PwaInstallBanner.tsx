...
export default function PwaInstallBanner() {
  ...
  const [modo, setModo] = useState<'ios' | 'android' | 'notif'>('ios');
  ...
  const atualizarEstado = useCallback(() => {
    ...
    if (appInstalada() && pushSuportado() && Notification.permission !== 'granted') {
      setModo('notif');
      setVisivel(true);
      return;
    }
  ...
  useEffect(() => {
    if (!appInstalada() || !pushSuportado()) return;
  ...
  async function ativarNotificacoes() {
    ...
    try {
      const ok = await registrarPushNotificacoes(true);
      if (ok) fechar();
    } finally {
          ...
          {modo === 'ios' && (
            <>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.45, mb: 1 }}>
                ...
                Vision Check e permita notificações.
              </Typography>
            </>
          )}
