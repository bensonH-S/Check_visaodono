...
.SYNOPSIS
  ...
  Zip:   Desktop\Meridian-BKOffice-Gerencia.zip (opcional)

  Uso:  .\build-kit-gerencia.ps1
        .\build-kit-gerencia.ps1 -SkipZip
#>
param(
  [switch]$SkipZip,
  [string]$OutDir = ''
...
if (Test-Path $OutDir) {
  ...
  try {
    ...
    $bak = "$OutDir-old-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
    Write-Host "Pasta travada - renomeando para $bak"
    Rename-Item $OutDir $bak -ErrorAction Stop
  }
}
$Runtime = Join-Path $OutDir 'runtime'
$App = Join-Path $OutDir 'app'
New-Item -ItemType Directory -Force -Path $OutDir, $Runtime, $App, (Join-Path $OutDir 'Logs'), (Join-Path $OutDir 'Logs\lojas') | Out-Null
Set-Content -Path (Join-Path $OutDir 'Logs\lojas\LEIA-ME.txt') -Encoding UTF8 -Value @"
Um arquivo de log por loja, por mes.
Ex.: 30797-terraco-shopping-2026-08.log
O worker cria o indice em _indice.txt no primeiro boot.
Nao use bkoffice-python-service.log — isso era o log unico antigo.
"@
...
if (-not $SkipZip) {
  ...
  Write-Host 'SkipZip: pasta pronta sem compactar.'
}

# Marca dias 01 ate ontem ja sincronizados (evita reimport no 1o boot)
$dataDir = Join-Path $OutDir 'data'
New-Item -ItemType Directory -Force -Path $dataDir | Out-Null
$hoje = (Get-Date).Date
$dias = @()
for ($d = Get-Date -Year $hoje.Year -Month $hoje.Month -Day 1; $d -lt $hoje; $d = $d.AddDays(1)) {
  $dias += $d.ToString('yyyy-MM-dd')
}
@{
  nota = 'historico ja no Meridian - so atualiza hoje'
  dias = $dias
  atualizado_em = (Get-Date).ToString('o')
} | ConvertTo-Json | Set-Content (Join-Path $dataDir 'synced-days.json') -Encoding UTF8
