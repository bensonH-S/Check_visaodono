...
import { carregarAliasesBkn } from '../services/bkoffice/bknAlias.js';
import {
  adquirirLease,
  liberarLease,
  statusLease,
...
router.post('/estoque/vendas-import', requireKitToken, upload.single('arquivo'), async (req, res, next) => {
  try {
    ...
    const bkAliases = [];
    const bknAtual = bkNumber ? String(bkNumber).replace(/\D/g, '') : '';
    for (const [antigo, atual] of aliases) {
      if (atual === bknAtual) bkAliases.push(antigo);
    }
